...
Continuous sinusoidal-wave thermal stimulation delivered via a 5-zone TCS thermode during fMRI scanning. Designed for thermal population receptive field (pRF) mapping, comparing **TGI** (Thermal Grill Illusion) and **Non-TGI** spatial patterns.
...
- **Stimulation**: sinusoidal waveform with 20 C amplitude (temperature range: 10-50 C)
- **VAS ratings**: 3 questions (cold, warm, burning intensity) with 8s timeout each

### Sinusoidal Waveform

Each 80s cycle contains **two full sine half-periods** (40s each):
...
fMRI_sinusoidal_wave/
  ...
  waveform.py        — Sinusoidal wave generation + phase shifting + mask application
