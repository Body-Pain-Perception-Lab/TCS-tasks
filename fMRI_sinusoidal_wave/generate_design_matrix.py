#!/usr/bin/env python3
"""
OBSOLETE — DO NOT USE FOR ANALYSIS.

This script models a UNIPOLAR sinusoidal waveform (40s period, 0.025 Hz)
but the actual thermode delivers a BIPOLAR wave (80s period, 0.0125 Hz).
The GLM regressors it produces do not match the delivered stimulus.

If GLM analysis is needed in future, this script must be rewritten to use
the bipolar waveform from waveform.py:generate_delta_waveform().
...
GLM regressors (HRF-convolved, at TR resolution):
    ...
    delta_centered   — sinusoidal waveform amplitude (mean-centered)
...
def sine_delta(t, period, max_delta):
    """Sinusoidal wave: 0 -> max_delta -> 0, repeating with given period."""
    return max_delta * np.sin(np.pi * (t % period) / period)
...
def generate_run_design(config, mask_name, warm_first, n_volumes=None,
    ...
    period = config['cycle_duration'] / 2.0  # 40s sine half-period
