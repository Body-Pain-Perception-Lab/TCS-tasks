...
if __name__ == '__main__':
    from config_electric_L01 import CONFIG as _CFG

    parser = argparse.ArgumentParser(
        description='Interactive DS5 single-pulse testing')
    parser.add_argument('--port', default=None,
                        help=f'Serial port (default from config: {_CFG["com_port"]})')
    parser.add_argument('--sim', action='store_true', default=None,
                        help='Simulation mode (no hardware)')
    parser.add_argument('--pw', type=float, default=None,
                        help=f'Pulse width in ms (default: {_CFG["pulse_width_ms"]})')
    parser.add_argument('--start', type=float, default=None,
                        help=f'Starting amplitude in mV (default: {_CFG["start_amplitude_mv"]})')
    parser.add_argument('--step', type=float, default=None,
                        help=f'Amplitude step size in mV (default: {_CFG["step_mv"]})')
    args = parser.parse_args()

    run_test(
        port=args.port if args.port else _CFG['com_port'],
        simulation=args.sim if args.sim is not None else _CFG['simulation'],
        pulse_width_ms=args.pw if args.pw else _CFG['pulse_width_ms'],
        start_mv=args.start if args.start else _CFG['start_amplitude_mv'],
        step_mv=args.step if args.step else _CFG['step_mv'],
    )
